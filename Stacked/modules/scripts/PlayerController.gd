...
func _physics_process(delta):
	...
	if DEBUG_MODE:
		$DebugUI/Panel/LabelsContainer/SpeedLabel.text = "Speed: %.2f" % velocity.length()
		$DebugUI/Panel/LabelsContainer/GoldLabel.text = "Gold: %.2f" % $Inventory.gold
		#print("slide check is colliding: ", slide_check.is_colliding())
